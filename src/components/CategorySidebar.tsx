...
function SortableGroup({ g }: { g: Group }) {
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <AccordionItem value={g.slug}>
        <div className="flex items-center gap-1">
          <button
            type="button"
            className="cursor-grab rounded p-1"
            {...attributes}
            {...listeners}
          >
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    setOrderedGroups((prev) => {
      ...
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify(next.map((g) => g.slug))
      );
    ...
    <aside className="hidden lg:block w-64 shrink-0">
      <div className="sticky top-20 rounded-lg border bg-card">
        ...
        <nav className="p-2">
          ...
          >
            <SortableContext
              items={orderedGroups.map((g) => g.slug)}
